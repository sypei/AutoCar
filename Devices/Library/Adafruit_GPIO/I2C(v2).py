...
import SensorLibrary.Adafruit_GPIO.Platform as Platform
...
class Device(object):
	...
	def __init__(self, address, busnum, i2c_interface=None):
		...
		if i2c_interface is None:
			...
			import SensorLibrary.Adacfruit_PureIO.smbus
			self._bus = SensorLibrary.Adafruit_PureIO.smbus.SMBus(busnum)
		else:
    ...
    def writeList_addr2(self, register, data):
	    """write bytes to AT24C32_address in real time clock"""
	    self._bus.write_i2c_block_data(self._at24c32_addr, register, data)
	    self._logger.debug("Wrote to register 0x%02X: %s",
				 register, data)
